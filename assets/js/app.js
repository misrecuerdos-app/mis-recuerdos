...
function getGoogleIdentity() {
  ...
  return {
    guestGoogleId: String(user.id || "").trim(),
    guestName: String(user.name || "").trim(),
    uploaderEmail: getUploaderEmail()
  };
}
...
function openSideMenu() {
  ...
  menu.innerHTML = `
    ...
    <aside class="side-menu-panel" aria-label="Menú principal">
      ...
      <section class="install-menu-card">
        <strong>Instalar aplicación</strong>
        <span>Ábrela desde tu pantalla de inicio, sin la barra del navegador.</span>
        <button class="session-action primary" data-install-app onclick="installRecuerdosApp()">Instalar aplicación</button>
      </section>
  ...
  if (typeof updateInstallButtons === "function") updateInstallButtons();
...
const INFO_TOPICS = {
  ...
  install: {
    title: "Cómo instalarla",
    body: `
      <p><strong>iPhone:</strong> abre la app en Safari, toca Compartir y elige Agregar a pantalla de inicio.</p>
      <p><strong>Android:</strong> abre la app en Chrome y toca Instalar aplicación; también aparece en el menú de tres puntos.</p>
      <button class="info-install-button" data-install-app onclick="installRecuerdosApp()">Instalar aplicación</button>
    `
  },
  guide: {
    title: "Cómo usarla",
  ...
  faq: {
    ...
    body: `
      ...
      <p>Sí, desde Mis Subidas y usando el mismo dispositivo.</p>
    `
  },
  privacy: {
    title: "Privacidad",
      ...
      <section class="info-page">
        ...
        ` : `
          ...
          <div class="info-index">
            ...
            <button onclick="openInfoTopic('install')"><span>Cómo instalarla</span><b>›</b></button>
...
async function loadMineGrouped() {
  ...
  try {
    const response = await fetch(
      `${UPLOAD_ENDPOINT}?action=mine&deviceToken=${encodeURIComponent(AppState.device.token)}&_=${Date.now()}`
    );
